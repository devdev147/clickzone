...
def updateARow(rowDataList,param): #rowDataList is a series
        ...
        try:
                ...
                frameUpdateTable.configure(background='#ffc0f3')
                r = 0
                for colname in rowDataList.index:
                        pk = param['pk'][0]
                        #print(i,' -- ',rowDataList[colname])
                        Label(frameUpdateTable, text=colname, bg='#ffc0f3') \
...
def extra():
    ...
    def insertIntoTableButtonClickEvent():
           ...
           try:
                   ...
                   frameInsertIntoTable.configure(background='#ffc0f3')
                   total_rows = df.shape[0]
                   r = 0
                   for i in range(total_rows):
                           '''
                           colname = df[i]
                           Label(frameInsertIntoTable, text=colname, bg='#ffc0f3') \
                                                           .grid(row=r, column=1, padx=15, pady=5, sticky='E')
                   ...
                   insertIntoTableSubmitButton = Button(frameInsertIntoTable, text="Insert into table", command=insertIntoTableSubmitButtonClickEvent)
...
def createInsertDataInputFRM(insertframe, table, param={}, r=0, c=1):
        ...
        insertDataInputFRM.configure(background='#ffc0f3')
        try:
                   ...
                   for i in range(total_rows):
                           '''
                           colname = df[i]
                           Label(insertDataInputFRM, text=colname, bg='#ffc0f3') \
                                                           .grid(row=r, column=1, padx=15, pady=5, sticky='E')
                           ...
                           if colname == 'id':
                                   ...
                                   if colname in cbocolslst:
                                           ...
                                           cbotbl = cbo[:cbo.find('.')]
                                           cbocol = cbo[cbo.find('.')+1:]
                                           print("cbo: ",cbo," cbotbl: ",cbotbl," cbocol: ",cbocol)
                                           colvals = []
                                           try:
                                           ...
                                           def funcSelectedEvent(event):
                                                   ...
                                                   # autofill
                                                   afv = ''
                                                   for afvi in range(len(param['autofill'])):
                                                                                                                      
                                                           afv = param['autofill'][afvi]

                                                           cboname = afv[:afv.find('#')]

                                                           #print("event.widget._name: ",event.widget._name," cboname+'CBO': ",cboname+'CBO')

                                                           if event.widget._name == cboname:
                                                                   autofill = afv[afv.find('#')+1:]                                                           
                                                                   autofilltbl = autofill[:autofill.find('.')]
                                                                   autofillcol = autofill[autofill.find('.')+1:]                                                           
                                                                   try:
                                                                           sql = "select "+autofillcol+" from "+autofilltbl                                                           
                                                                           sql += " where "+event.widget._name                                                           
                                                                           sql += " = '"+val+"'"

                                                                           print(sql)

                                                                           cursor.execute(sql)
                                                                           autofillval = cursor.fetchone()[autofillcol]

                                                                           print(autofillval)

                                                                           globals()[autofillcol+'TXT'].delete(0, END)
                                                                           globals()[autofillcol+'TXT'].insert(INSERT, autofillval)                                                           
                                                                   except conn.Error as e:
                                                                           msg = "ERROR: "+str(e.args[0])+e.args[1]
                                                                           tk.messagebox.showinfo("MESSAGE", msg, parent=insertDataInputFRM)
...
def createInsertChildFrames(insertrootframe, param):
        table = param['table'][0]
        pk = param['pk'][0]
        cbos = ', '.join(param['cbo']) #explode list to comma separated strings
        ...
        Label(frame, text='TABLE:').grid(row=0, column=0, sticky=W)
        Label(frame, text=table).grid(row=0, column=1, sticky=W)
        Label(frame, text='PRIMARY KEY:').grid(row=1, column=0, sticky=W)
        Label(frame, text=pk).grid(row=1, column=1, sticky=W)
        Label(frame, text='LOOKUP VALUES (CBO):').grid(row=2, column=0, sticky=W)
        Label(frame, text=cbos).grid(row=2, column=1, sticky=W)
...
def createInsertRootFrame(rootframe, param):
        ...
        insertrootframe.configure(background='#FFEFD5')
