...
def updateARow(rowDataList): #rowDataList is a series
        try:
                ...
                frameUpdateTable.configure(background='#c7b6d4')
                r = 0
                for colname in rowDataList.index:
                        #print(i,' -- ',rowDataList[colname])
                        Label(frameUpdateTable, text=colname, bg='#c7b6d4') \
...
def createChartFRM(root, df, xcol, ycol, charttype):
        ...
        #h = h*5/100
        r = 0
        chartFRM = Frame(root, width=w, bg='#e8c1e3')#, height=h)
...
def createPlotFRM(frame, plotparams):
        ...
        plotFRM.configure(background="#e8c1e3") #"light grey"
